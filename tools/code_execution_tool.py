...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    try:
        ...
        from tools.environments.local import scrub_delegated_child_ownership_env

        if is_delegated_child_process_context():
            # fork(tars) 30.07.: nur die Eigentums-Marker, nicht die ganze
            # Kanban-Env — siehe tools/environments/local.py.
            scrubbed = scrub_delegated_child_ownership_env(scrubbed)
    except Exception:
        pass
    return scrubbed
