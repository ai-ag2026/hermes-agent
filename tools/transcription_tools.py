...
from hermes_cli.audio_key_guard import (
    ...
    # fork(tars) 30.07.: umbenannt. Upstream definiert seit diesem Merge
    # ein EIGENES _resolve_provider_key(env_var, provider_id) und ruft es
    # an ~10 Stellen zweiargumentig auf — unser Alias hätte das still
    # überschrieben und jeden dieser Aufrufe in einen TypeError laufen
    # lassen. Direktive: bei Namenskollision benennen WIR um.
...
def _transcribe_openai(
    ...
    *,
    ...
    # fork(tars): ``language or`` statt purem Überschreiben. Upstreams Kette
    # (stt.<provider>.language > stt.language > env > Vorgabe) bleibt der
    # Normalfall, aber ein vom Aufrufer AUSDRÜCKLICH übergebener Wert gewinnt —
    # sonst wäre der Parameter tot und der Selbsthosting-Pfad könnte die
    # Sprache nicht mehr pro Aufruf setzen.
    ...
    try:
        ...
        # fork(tars): Timeout klemmen statt fest 30 s — selbst gehostete
        # CPU-Server brauchen mehr, und response_format bleibt überschreibbar,
        # weil manche nur ein Format implementieren. Alles andere ist upstreams.
        ...
        def _create_transcription(path: str):
            with open(path, "rb") as audio_file:
                create_kwargs = {
                    ...
                    or ("text" if model_name == "whisper-1" else "json"),
                }
                if language:
...
def _transcribe_prepared_audio(file_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    if provider == "openai":
        ...
        return _transcribe_openai(
            ...
            # fork(tars): kein language= mehr. Upstream löst die Sprache seit
            # diesem Merge in _transcribe_openai selbst über
            # _resolve_stt_language(provider_label) auf — Kette
            # stt.<provider>.language > stt.language > env > auto, also eine
            # Obermenge unseres alten openai_cfg["language"]. Ein hier
            # durchgereichter Wert würde dort ohnehin überschrieben.
