...
def test_reconcile_and_repair_route_to_different_implementations(cli_home, capsys):
    ...
    # Merge 30.07.: unser Reconciler heißt seit 4f86c2236 `reconcile`
    # (Direktive: bei Namenskollision benennen WIR um) — `repair` gehört
    # jetzt ungeteilt upstreams DB-Integritätspfad. Der alte Doppelrouting-
    # Vertrag (nacktes `repair` → Reconciler) ist damit Geschichte.
    rc = _run_kanban_cli(["reconcile"])
    out_reconciler = capsys.readouterr().out
    assert rc == 0
    assert "DRY-RUN" in out_reconciler, "`reconcile` must stay the dry-run invariant reconciler"
