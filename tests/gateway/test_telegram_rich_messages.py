...
async def test_legacy_send_error_redacts_bot_token_without_traceback(monkeypatch, caplog):
    ...
    assert result.error is not None
    assert token not in result.error
    # Real PTB TelegramError capitalize()s its message (fake mock did not) —
    # assert the redaction case-insensitively; the property under test is
    # the token redaction, not the URL casing.
