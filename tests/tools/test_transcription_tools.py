...
class TestSelfHostedOpenAISTT:
    ...
    def test_language_falls_back_to_resolver_default(self, monkeypatch, sample_wav):
        """Ohne ausdrückliche Sprache greift upstreams Auflösungskette.

        Bis zum Merge 30.07. schickte der Aufruf gar kein ``language``.
        Upstream setzt jetzt eine Vorgabe (siehe test_stt_default_language),
        weil ein Sprachhinweis die Trefferquote deutlich hebt.
        """
        mock_client = MagicMock()
        mock_client.audio.transcriptions.create.return_value = "hi"
        with patch("tools.transcription_tools._HAS_OPENAI", True), \
            ...
            _transcribe_openai(sample_wav, "m", api_key="k", base_url=self.SELF)
        kwargs = mock_client.audio.transcriptions.create.call_args.kwargs
        assert kwargs.get("language"), "resolver muss eine Vorgabesprache liefern"

    def test_transcribe_audio_threads_language_and_config(self, monkeypatch, sample_wav):
        """End-to-end dispatch: die konfigurierte Sprache erreicht den
