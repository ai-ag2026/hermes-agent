...
def test_kanban_tools_visible_with_env_var(monkeypatch, tmp_path):
    ...
    expected = {
        ...
        # upstream attachment toolset (merge 2026-07-17)
...
def test_link_happy_path_orchestrator(worker_env, monkeypatch):
    ...
    monkeypatch.delenv("HERMES_KANBAN_TASK", raising=False)
    monkeypatch.delenv("HERMES_KANBAN_RUN_ID", raising=False)
# fork(tars) 31.07.2026: upstreams test_link_happy_path ist hier ENTFALLEN.
# Es lässt einen WORKER zwei fremde Tasks verknüpfen — unser Fork verbietet
# genau das seit dem Audit 2026-07-27 (Kanten nur mit eigener Task; Familie
# attach/link). Beide Seiten unseres Vertrags sind direkt darüber geprüft:
# test_link_happy_path_worker_own_task_endpoint (erlaubt) und
# test_link_rejects_foreign_edge_for_worker (verboten); der Orchestrator-Fall
# in test_link_happy_path_orchestrator.
